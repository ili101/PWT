...
if ($Config['Components']['SQLite']['AdminPage']) {
    Add-PodeWebPage -Name 'Admin' -Icon settings -ScriptBlock {
        New-PodeWebTable -Name 'Explorer' -Id 'AdminUsers' -DataColumn Name -Filter -Sort -Click -Paginate -ScriptBlock {
            $Users = Invoke-Sql -QueryPath '\Components\SQLite\User\ItemList.sql' -Stream
            # wait-debugger
            $Users
        }
    }
